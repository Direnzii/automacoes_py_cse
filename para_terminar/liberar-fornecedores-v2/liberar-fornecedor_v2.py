...
class liberarForn:
    ...
    def validar_listas(self, cliente, liberar=False):
        matriz = cliente in self.lista_matrizes
        filial = cliente in self.lista_filiais
        nao_cliente = cliente in self.lista_nao_clientes
        condicao = str(cliente) in self.abrir_arquivo('cnpjs_filial.txt')
        condicao2 = str(cliente) in self.abrir_arquivo('cnpjs_nao_cliente.txt')
        condicao3 = str(cliente) in self.abrir_arquivo('cnpjs_com_distribuidor_cadastrado.txt')
        condicao4 = str(cliente) in self.abrir_arquivo('cnpjs_matrizes.txt')
        if condicao3:
            return True
        if matriz == True:
            return True
            ...
            for numero in range(1, contar_filiais + 1):  # range usado para usar um for na quantidade de filiais
                try:
                    ...
                    if validacao_filial == False:
                        ...
                        if not self.validar_listas(int(filial)):
    ...
    def verificar_chat(self, site_verificar_chat):
            ...
            if validar_se_e_cliente_cote == True:
                ...
                if linha_cliente_ver == True:
                    try:
                        ...
                        if contador_cnpj_faltando_numero >= 10:  # Caso o cnpj que esteja aparecendo for diferente do pesquisado
                            return True
                        time.sleep(2)
    ...
    def reiniciar_a_tela(self, cliente, liberar=False):
        if not liberar:
    ...
    def validar_se_da_pra_ver_loading(self):
        ...
        if contador_exception_loading >= 8:  # Considerei 8 tentativar um numero razoavel, pois pode acontecer de ser só lentidão mesmo
            site.close()
            return 'Error_close'
        da_para_ver = True
    ...
    def organizar_listas(self):
        while True:
            try:
                ...
                arquivo = self.abrir_arquivo(nome_arquivo)  # abrir arquivo
                ...
                for cliente in arquivo:  # percorrer a lista de clientes (arquivo)
                    ...
                    if mega_validacao >= 10:
                        ...
                        if not validacao:
                            # time.sleep(4)
                            validacao_chat = self.verificar_chat(site)
                            if validacao_chat:
                            ...
                            if validacao_cliente_cotefacil:
                                ...
                                if da_para_ver_botao_ativar_inativar:
                                    ...
                                    if not input_matriz:
                                        ...
                                        if not self.validar_listas(int(cliente)):
                                            with io.open('cnpjs_filial.txt',
                                                         'a', encoding='utf-8') as file:
                                                file.write(f'{int(cliente)}\n')
    ...
    def liberar_fornecedor(self, lista_matriz):
        global validar_se_checkbox_filiais_ta_checada
        for cliente in lista_matriz:
            if not self.validar_listas(cliente, liberar=True):
                self.reiniciar_a_tela(cliente, liberar=True)
